...
(function(module) {
  ...
  Article.numWordsAll = () => {
    return Article.all.map(article => article.body).reduce((acc, val) => acc + val.split(' ').length, 0);
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(author => author.author).reduce(function(names, name) {
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      ...
      return {
        author: author,
        numWords: Article.all.filter(article => article.author === author).map(words => words.body).reduce((acc, val) => acc + val.split(' ').length, 0)
      }
